...
public class UserServiceTest {
    ...
    public void testCreateIncorrectUserData() {
        ...
        Assertions.assertThrows(IncorrectDataException.class, () -> userService.create(createdDTO));
        int after = userService.findAll().size();
    ...
    public void testUpdatedIncorrect() {
        ...
        user2.setFirstName("");
        user2.setLastName("");
        Assertions.assertThrows(IncorrectDataException.class, () -> userService.update(user2, user.getId()));
        User foundUser = userRepository.findById(user.getId()).orElse(null);
        Assertions.assertNotNull(foundUser);
        Assertions.assertEquals(user.getFirstName(), foundUser.getFirstName());
    }
